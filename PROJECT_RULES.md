...
修正版依次使用 `V2`、`V3`。不要再使用旧的 `产品名_V.xlsx` 或 `产品名_V2.xlsx` 命名方式，除非用户明确要求。

## 变体前台展示规则

建立父子体时，除了模板能否上传，还必须考虑 Amazon 买家端的变体展示体验。参考 Amazon Seller Central 官方帮助页 `G7UJDST28Q5Y7PWY`（Variation display and customer experience）：

- 变体维度是买家可选择的选项，例如尺寸、颜色、款式。
- 颜色、图案等视觉维度可能会让详情页图片随所选子体变化；尺寸等非视觉维度通常不会改变图片。
- 除服装、配饰、箱包、体育用品、美妆等特殊分类外，大多数分类在搜索结果和详情页默认展示最畅销子体；买家再通过下拉菜单或商品图片切换同一变体系列的其他子体。
- 子体展示顺序由 Amazon 系统自动决定，不能通过模板字段人工指定。
- 如果业务目标是让买家在一个变体选项里直接看到“款式名 + 尺寸”，优先把变体主题设为可承载完整选项文案的单一维度（如模板支持的 `SET_NAME`），并在对应字段写入 `Style Name Size` 形式的英文值，例如 `WARNING Print 1.97 in x 118.11 in`。
- 只有当需要买家分别选择两个维度时，才使用组合维度（如 `SIZE/SET_NAME`）；组合维度可能在前台拆成两个选择项，而不是一个“款式 + 尺寸”的按钮。
- 不要为了触发父体标题规则而虚构颜色差异。若本质是款式差异，父体标题使用 `Multiple Styles Available`，不要写 `Multiple Colors Available`。
