...
9. 输出文件只保留填写好的上传表格，命名为 `中文产品名款式颜色V1.xlsx`，例如 `卷纸切割器黑色V1.xlsx`；根据错误报告修正后的版本依次为 `中文产品名款式颜色V2.xlsx`、`中文产品名款式颜色V3.xlsx`。
```
...
- 填好的上传表使用中文商品名 + 款式/颜色 + 版本号，版本号前不加下划线，例如 `卷纸切割器黑色V1.xlsx`。
- 修正版按 `V2`、`V3` 递增。
- 不再默认使用旧格式 `产品名_V.xlsx`、`产品名_V2.xlsx`。
...
父子体不仅要满足模板校验，还要判断买家在详情页实际看到的变体选择方式。参考 Amazon Seller Central 官方帮助页 `G7UJDST28Q5Y7PWY`：

- 变体维度就是买家端可选择的选项，例如尺寸、颜色、款式。
- 视觉维度（颜色、图案等）可能会切换详情页图片；非视觉维度（尺寸等）通常不改变图片。
- 大多数非服装/配饰/箱包/体育/美妆分类会展示最畅销子体，买家通过下拉菜单或图片切换其他子体。
- 子体顺序由 Amazon 自动排序，模板里不能人工指定前台顺序。
- 如果要让买家在一个选项里看到“款式名 + 尺寸”，优先使用模板支持的单一文本维度，例如 `SET_NAME`，并把值写成 `WARNING Print 1.97 in x 118.11 in` 这种英文前台文案。
- 如果使用 `SIZE/SET_NAME` 等组合维度，要预期前台可能拆成两个选择维度：尺寸和款式。
- 款式变体不要强行写成颜色变体；父体标题按真实差异写 `Multiple Styles Available`，不要因为子体字段颜色不同就写 `Multiple Colors Available`。
...
- 文件能被 WPS/Excel 打开
- 正式 `auto-fill` 路线首次生成 V1 后，必须用 WPS/Excel 实际打开 `Template` 页，让模板动态条件格式重新计算；逐段检查新增红框，补齐后覆盖同一个 V1。显式的“批量快速上品路线”可以先使用 `check-red-fields` 计算当前联动红框；只有扫描器遇到未支持公式、新 Product Type/模板变化或后台报错时，才回退到 WPS/Excel。这个交付前内部修正不递增为 V2，只有已交付文件或错误报告后的修正版才递增版本。
- `Template` 页第 7 行起有数据
- 父体 SKU / 子体 SKU 正确
- 子体 Parent SKU 指向父体
- Product Type 正确
