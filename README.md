...
```bash
python3 run.py batch-workbench
```

默认打开 `http://127.0.0.1:8767/batch-fast`。页面记录批量清单、竞品 HTML、原始模板和输出文档；既可单个下载，也可将当前批次已有的全部输出文档打包为 ZIP 批量下载到本地。输出文档由系统生成后自动同步，页面不提供人工上传入口。输出默认从 V1 开始；生成 V2/V3 后自动将最高版本设为当前输出，旧版保留单独下载入口，批量 ZIP 只包含每款商品的当前最新版。

业务文件保存在 `data/batch_fast_workbench/`，台账使用仓库相对路径，因此该目录提交到 Git 后，同事更新同一个仓库即可看到相同记录并下载文件。新增的桌面文件在提交前执行一次：

```bash
python3 run.py package-batch-workbench-data
```
